...
class W3MImageDisplayer(ImageDisplayer):
    ...
    def draw(self, path, start_x, start_y, width, height, scale_h=False):
        if not self.is_initialized or self.process.poll() is not None:
            self.initialize()
        self.process.stdin.write(self._generate_w3m_input(path, start_x,
            start_y, width, height, scale_h))
    ...
    def _generate_w3m_input(self, path, start_x, start_y, max_width, max_height, scale_h):
        ...
        if height > max_height_pixels or scale_h and height < max_height_pixels and width < max_width_pixels:
